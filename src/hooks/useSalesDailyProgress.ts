...
export function useSalesDailyProgress() {
  ...
  const [period, setPeriod] = useState<SalesProgressPeriod>(todayPeriod);
  ...
  const desde = period.desde.slice(0, 10);
  const hasta = period.hasta.slice(0, 10);
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: payload, error: rpcError }, { data: stock }] = await Promise.all([
      supabase.rpc('get_sales_daily_progress', range),
      supabase.rpc('get_sales_progress_boss_stock', range),
    ]);
    ...
    setIsLoading(false);
  }, [supabase, user, desde, hasta, vendedorId]);
  ...
  const openCategory = useCallback(
    ...
    [supabase, user, desde, hasta, vendedorId, data?.vendedor_id]
  );
