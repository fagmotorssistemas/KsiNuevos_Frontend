...
import { Plus, Search, Users, Phone, CreditCard, Edit2, Trash2, Calendar } from "lucide-react";
...
export default function PersonalPage() {
    const { empleados, isLoading, guardarEmpleado, eliminarEmpleado } = usePersonal();
    ...
    const handleDelete = async (id: string) => {
        if (confirm("¿Estás seguro de eliminar esta ficha de personal?")) {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Personal del taller</h1>
                    <p className="text-slate-500">Constancia de empleados: nombre, contacto, cargo y nómina.</p>
                </div>
                <button
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filtered.map((empleado) => (
                        <div
                            key={empleado.id}
                            className={`bg-white rounded-2xl border-2 p-6 transition-all group relative ${
                                !empleado.activo
                        ...
                        >
                            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button
                                    ...
                                    className="p-2 bg-slate-100 hover:bg-blue-50 text-slate-500 hover:text-blue-600 rounded-lg"
                                >
                                    <Edit2 className="h-4 w-4" />
                                </button>
                                <button
                                    onClick={() => handleDelete(empleado.id)}
                                    className="p-2 bg-slate-100 hover:bg-red-50 text-slate-500 hover:text-red-600 rounded-lg"
                                >
                            ...
                            <div className="flex items-center gap-4 mb-4">
                                <div
                                    className={`h-14 w-14 rounded-full flex items-center justify-center text-xl font-bold shadow-sm ${
                                        empleado.activo
                            ...
                            <div className="space-y-3 text-sm border-t border-slate-100 pt-4">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">
                                        ${(empleado.salario_mensual || 0).toLocaleString()}
                                    </span>
                                </div>

                                {empleado.telefono && (
                                    <div className="flex items-center gap-2 text-slate-500 pt-2">
                                        <Phone className="h-4 w-4" /> {empleado.telefono}
                                    </div>
                                )}
                ...
                <div className="p-16 text-center bg-white rounded-2xl border border-slate-200">
                    ...
                    <p className="text-slate-500 mt-1">
                        Registra a las personas del taller para llevar constancia de nómina y contacto.
                    </p>
                </div>
            )}
