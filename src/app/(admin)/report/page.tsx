...
import { useAdminStats } from "@/hooks/useAdminStats"; // Importamos el Hook actualizado
import { AdminToolbar } from "@/components/features/admin/AdminToolbar"; // Importamos el Toolbar actualizado
import { 
    Calendar, 
    MessageCircle, 
    MapPin, 
...
export default function AdminDashboardPage() {
    ...
    // Usamos el Hook actualizado que ahora devuelve customDate/setCustomDate
    const {
        stats,
        isLoading,
        dateFilter,
        setDateFilter,
        customDate,      // <--- Modificado
        setCustomDate,   // <--- Modificado
        reload
    } = useAdminStats();

    // Calculamos los totales globales
    const grandTotals = useMemo(() => stats.reduce((acc, curr) => ({
    ...
    }), { leads: 0, showroom: 0, appointments: 0, requests: 0, proformas: 0 }), [stats]);
    ...
    return (
        <div className="p-6 space-y-6 bg-slate-50 min-h-screen font-sans">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 animate-in fade-in slide-in-from-top-4 duration-500">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Panel de Rendimiento</h1>
                    <p className="text-slate-500 mt-1 flex items-center gap-2">
                        <TrendingUp className="h-4 w-4 text-brand-500" />
                        Monitoreo de productividad diaria de asesores
                    </p>
                </div>
            </div>

            {/* --- TOOLBAR --- */}
            <div className="sticky top-0 z-10 backdrop-blur-md bg-white/80 rounded-xl shadow-sm border border-slate-200/60">
                <AdminToolbar 
                    currentFilter={dateFilter}
                    onFilterChange={setDateFilter}
                    customDate={customDate}           // <--- Modificado
                    onCustomDateChange={setCustomDate} // <--- Modificado
                    onRefresh={reload}
                    isLoading={isLoading}
                />
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                ...
                <StatCard 
                    icon={CalendarCheck} 
                    label="Citas" 
                    value={grandTotals.appointments} 
                                ...
                                <tr>
                                    <td colSpan={7} className="py-12 text-center text-slate-500">
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <span>No hay actividad registrada en este rango de fechas.</span>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                stats.map((seller) => (
                                    <tr key={seller.profile_id} className="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <td className="py-4 px-6 text-center">
                                            <span className={`inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-xs font-medium min-w-[30px] ${seller.leads_responded > 0 ? 'bg-blue-100 text-blue-700' : 'text-slate-300'}`}>
                                                {seller.leads_responded}
                                            </span>
                                        </td>
                                        <td className="py-4 px-6 text-center">
                                            <span className={`inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-xs font-medium min-w-[30px] ${seller.showroom_created > 0 ? 'bg-purple-100 text-purple-700' : 'text-slate-300'}`}>
                                                {seller.showroom_created}
                                            </span>
                                        </td>
                                        <td className="py-4 px-6 text-center">
                                            <span className={`inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-xs font-medium min-w-[30px] ${seller.appointments_created > 0 ? 'bg-green-100 text-green-700' : 'text-slate-300'}`}>
                                                {seller.appointments_created}
                                            </span>
                                        </td>
                                        <td className="py-4 px-6 text-center">
                                            <span className={`inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-xs font-medium min-w-[30px] ${seller.requests_created > 0 ? 'bg-orange-100 text-orange-700' : 'text-slate-300'}`}>
                                                {seller.requests_created}
                                            </span>
                                        </td>
                                        <td className="py-4 px-6 text-center">
                                            <span className={`inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-xs font-medium min-w-[30px] ${seller.proformas_created > 0 ? 'bg-indigo-100 text-indigo-700' : 'text-slate-300'}`}>
