...
export default function ContratosPage() {
  const { contratos, loading } = useContratosData()
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 w-64 shadow-sm"
            value={searchTerm}
            ...
            {loading ? (
              ...
              filteredContratos.map((c) => (
                <tr key={c.ccoCodigo} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4 font-medium text-slate-900">{c.clienteNombre}</td>
                  <td className="px-6 py-4 text-slate-600 font-mono">{c.clienteId}</td>
