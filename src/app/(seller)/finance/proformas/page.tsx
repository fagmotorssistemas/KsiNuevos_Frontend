...
export default function CreditProformasPage() {
    ...
    const fetchProformas = useCallback(async () => {
        ...
        // Vendedores: solo las suyas. Admin: todas (o "solo las mías" si lo activa).
        if (!canSeeAll || onlyMine) {
        ...
        setIsLoading(false);
    }, [supabase, onlyMine, user?.id, canSeeAll]);
    ...
    const withPdfCount = useMemo(
        () => filtered.filter((p) => !!p.pdf_url).length,
        [filtered]
    );
    ...
    return (
        <div className="space-y-6">
            ...
            <div className={`grid grid-cols-2 ${canSeeAll ? "md:grid-cols-3" : "md:grid-cols-2"} gap-3`}>
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Con PDF</p>
                    <p className="text-2xl font-bold text-blue-600 mt-1">{withPdfCount}</p>
                </div>
                {canSeeAll && (
                    <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm col-span-2 md:col-span-1">
                        <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Filtro</p>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-2 w-full lg:w-auto">
                                        ...
                                        {item.pdf_url ? (
                                            ...
                                            <span className="flex-1 lg:flex-none inline-flex items-center justify-center gap-2 px-4 py-2 bg-slate-100 text-slate-400 rounded-lg text-sm font-medium cursor-not-allowed">
                                                Sin PDF
                                            </span>
                                        )}
                                    </div>
                ...
                {!isLoading && filtered.length > 0 && (
                    <div className="p-3 border-t border-slate-100 bg-slate-50 text-center text-xs text-slate-400">
                        ...
                        {proformas.length >= 300 ? " (máx. 300 más recientes)" : ""}
                    </div>
                )}
            </div>
        </div>
