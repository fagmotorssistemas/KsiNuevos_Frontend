    ...
    <div className="space-y-4">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-700">
              {flowType === 'single'
                ...
                : `Arrastra los clips (${files.length}/${VIDEO_MAX_CLIPS}) o haz clic para seleccionar`}
            </p>
      ...
      {files.length > 0 && (
        <div className="space-y-2">
          ...
          {flowType === 'multiple' && files.length < VIDEO_MAX_CLIPS && (
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
            ...
            >
              + Agregar más clips ({files.length}/{VIDEO_MAX_CLIPS})
            </button>
          )}
        </div>
      )}
