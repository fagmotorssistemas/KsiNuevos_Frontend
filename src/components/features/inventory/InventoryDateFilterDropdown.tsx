...
import { Calendar, CalendarClock, CalendarDays, CalendarRange, Check, ChevronDown, History } from 'lucide-react'
import {
    INVENTORY_DATE_FILTER_OPTIONS,
    type InventoryDateRange,
} from '@/lib/inventario/inventoryDateFilter'

const ICONS: Record<InventoryDateRange, typeof Calendar> = {
    all: History,
    today: Calendar,
    '7days': CalendarDays,
    '15days': CalendarClock,
    thisMonth: CalendarRange,
    custom: CalendarRange,
}

type Props = {
    value: InventoryDateRange
    onChange: (value: InventoryDateRange) => void
}

export function InventoryDateFilterDropdown({ value, onChange }: Props) {
    const [open, setOpen] = useState(false)
    const rootRef = useRef<HTMLDivElement>(null)
    const selected =
        INVENTORY_DATE_FILTER_OPTIONS.find((option) => option.value === value) ??
        INVENTORY_DATE_FILTER_OPTIONS[0]
    ...
    return (
        <div ref={rootRef} className="relative min-w-[210px]">
            ...
            {open ? (
                ...
                >
                    {INVENTORY_DATE_FILTER_OPTIONS.map((option) => {
