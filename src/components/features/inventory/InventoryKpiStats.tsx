...
export type InventoryKpiFilter = "all" | "active" | "baja";
...
interface InventoryKpiStatsProps {
    data: InventoryKpiSummary | null;
    loading: boolean;
    activeFilter: InventoryKpiFilter;
    onFilterChange: (filter: InventoryKpiFilter) => void;
}

export function InventoryKpiStats({
    data,
    loading,
    activeFilter,
    onFilterChange,
}: InventoryKpiStatsProps) {
    if (loading || !data) {
        return (
    ...
    const cardBaseClass =
        "p-6 rounded-xl border shadow-sm relative overflow-hidden transition-all duration-200 cursor-pointer hover:shadow-md transform hover:-translate-y-1";
