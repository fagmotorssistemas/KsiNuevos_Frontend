...
export function AppointmentCard({ appointment, onComplete, onCancel }: AppointmentCardProps) {
    const { title, start_time, location, lead, status } = appointment;
    ...
    const isPending = status === 'pendiente' || status === 'confirmada';
    ...
    if (status === 'completada') {
        ...
        bgClass = "bg-red-50/40"; // Fondo rojo suave para alertar
    ...
    return (
        <div className={`relative flex items-start gap-4 p-4 rounded-xl border border-slate-200 shadow-sm transition-all hover:shadow-md ${borderClass} ${bgClass}`}>
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="mt-1 space-y-1">
                    ...
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                        ...
                        <span className="font-medium">{lead?.name || 'Cliente Desconocido'}</span>
                        {lead?.phone && (
                            <>
                    ...
                    {/* Auto de Interés (Si tiene) */}
                    {lead?.interested_cars?.[0] && (
                        <div className="flex items-center gap-2 text-xs text-slate-500">
                            <Car className="h-3.5 w-3.5 text-slate-400" />
                            <span>{lead.interested_cars[0].brand} {lead.interested_cars[0].model}</span>
                        </div>
                    )}
            ...
            {/* Columna Acciones (Solo si está pendiente) */}
            {isPending && (
