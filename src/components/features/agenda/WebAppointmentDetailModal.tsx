...
import { ImageViewerModal } from '../inventory/ImageViewerModal';
...
export function WebAppointmentDetailModal({ 
    ...
    // OBJETO CORREGIDO PARA EVITAR ERRORES DE TYPESCRIPT
    const carForViewer = {
        // Convertimos el id numérico a string para cumplir con el tipo UUID esperado
        id: String(appointment.id), 
        brand: (isBuying ? buyInventory?.brand : sellRequest?.brand) || 'Vehículo',
        model: (isBuying ? buyInventory?.model : sellRequest?.model) || '',
        year: (isBuying ? buyInventory?.year : sellRequest?.year) || 2024,
        img_gallery_urls: photos,
        price: (isBuying ? buyInventory?.price : sellRequest?.client_asking_price) || 0,
        mileage: (isBuying ? buyInventory?.mileage : sellRequest?.mileage) || 0,
        transmission: (!isBuying ? sellRequest?.transmission : 'N/A') || 'N/A',
        // Usamos un valor permitido por el tipo 'location' o null
        location: (isBuying ? buyInventory?.location : 'otro') || 'otro', 
        status: 'disponible'
    } as InventoryCar; // Forzamos el casting ya que es un objeto temporal para el visor
                                        ...
                                        <div className="grid grid-cols-3 gap-2">
                                            {photos.slice(0, 3).map((url: string, idx: number) => (
                                                <img 
                                                    ...
                                                    src={url} 
                                                    alt={`Foto ${idx}`} 
                                                    className="aspect-square bg-slate-100 rounded-lg object-cover border border-slate-200 cursor-pointer hover:opacity-80 transition-all" 
