...
import type { SimulatorValues, SimulatorResults } from "@/hooks/useCreditSimulator";
...
export function CreditProforma({ values, results }: CreditProformaProps) {
        ...
        <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-lg print:shadow-none print:border-none print:p-0">
            ...
            <div className="flex justify-between items-start border-b-2 border-slate-800 pb-6 mb-8">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">Concesionaria XYZ | Financiamiento Directo</p>
                </div>
                <div className="text-right">
                    <div className="text-sm text-slate-500">Fecha de Emisión</div>
                    <div className="font-medium text-slate-900">{new Date().toLocaleDateString('es-EC', { dateStyle: 'long' })}</div>
                    {values.clientName && (
                        <div className="mt-2 text-blue-600 font-bold text-lg">{values.clientName}</div>
                    )}
                </div>
            </div>
            ...
            <div className="mt-8">
                ...
                <div className={`overflow-hidden transition-all duration-300 ${showTable ? 'opacity-100 max-h-[2000px]' : 'opacity-0 max-h-0 print:opacity-100 print:max-h-[5000px]'}`}>
            ...
            <div className="mt-12 pt-6 border-t border-slate-200 text-xs text-slate-400 text-center leading-relaxed print:mt-8">
                <p>Esta cotización es referencial y tiene una validez de 5 días. Precios y tasas sujetos a aprobación crediticia final.</p>
