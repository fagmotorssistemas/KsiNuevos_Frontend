...
import { ResumenEmpleados } from "@/types/employees.types";

interface EmployeesKpiStatsProps {
    data: ResumenEmpleados | null;
    loading: boolean;
}

export function EmployeesKpiStats({ data, loading }: EmployeesKpiStatsProps) {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            ...
            <div className="p-6 rounded-xl border border-emerald-100 bg-emerald-50/50 shadow-sm relative overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <p className="text-sm text-emerald-700 font-medium mt-2">
                        {(data.totalActivos / data.totalEmpleados * 100).toFixed(1)}% Operativo
                    </p>
                </div>
            </div>
            ...
            <div className="p-6 rounded-xl border border-violet-100 bg-violet-50/50 shadow-sm relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-10">
                    <Banknote className="h-24 w-24 text-violet-600" />
                </div>
                <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-4xl font-bold text-slate-900 tracking-tight">
                        {formatMoney(data.totalNominaMensual)}
                    </h3>
