...
const menuItems = [
    ...
    // Noticiero: oculto en sidebar hasta lanzamiento (la ruta /marketing/noticiero sigue existiendo).
            ...
            >
                <div className={`h-20 flex items-center border-b border-gray-100 transition-all duration-300 ${isCollapsed ? 'justify-center px-0' : 'justify-between px-6'}`}>
                    <div className={`flex items-center ${isCollapsed ? 'justify-center w-full' : ''} overflow-hidden`}>
                        <div className="min-w-[2.5rem] w-10 h-10 bg-violet-600 rounded-xl flex items-center justify-center text-white shadow-violet-200 shadow-lg">
                            <Megaphone size={20} />
                        </div>
                        ...
                        <div className={`ml-3 transition-opacity duration-300 ${isCollapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100'}`}>
                            ...
                            <p className="text-xs text-violet-600 font-semibold tracking-wide whitespace-nowrap">MARKETING</p>
                        </div>
                    </div>
                            ...
                            return (
                                ...
                                >
                                    <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'}`}>
                                        <item.icon
                                            size={20}
                                            className={`transition-colors shrink-0 ${isActive ? 'text-violet-600' : 'text-gray-400 group-hover:text-gray-600'}`}
                                        />
                                    ...
                                    {isActive && isCollapsed && (
                                        <div className="absolute right-1 top-1/2 -translate-y-1/2 w-1 h-8 rounded-full bg-violet-600" />
                                    )}
                                </Link>
                            );
                        })}
