...
export const gpsContratosService = {
        ...
        if (resumenApi.length > 0) {
            ...
            const promesasDetalle = resumenApi.map(async (item: any) => {
                try {
                    ...
                    if (!id) return null;
                    return await gpsContratosService.getDetalleContratoGPS(id);
                } catch (e) {
                    return null;
                }
            });
  ...
  // ==========================================
  // OBTENER DETALLE (Solo para Origen 'AUTO')
  // ==========================================
  async getDetalleContratoGPS(id: string): Promise<ContratoGPS | null> {
    try {
        ...
        // Cuota mensual (Oracle: cuotaMensual, cuota_mensual, valorCuota)
