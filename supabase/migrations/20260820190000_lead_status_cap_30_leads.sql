-- Tope de cambio de estado: 30 leads contestados/día (3 pts × 30 = 90).
-- Un mismo lead cuenta una sola vez.

UPDATE public.sales_progress_weights
SET daily_cap_points = 90
...
AS $$
  ...
  pipeline AS (
    SELECT unnest(ARRAY[
      ...
      'contactado'::public.lead_status,
      'interesado'::public.lead_status,
      'negociando'::public.lead_status,
      'en_proceso'::public.lead_status
    ]) AS st
  ),
  closed AS (
    SELECT unnest(ARRAY[
  ...
  SELECT h.changed_by, 'lead_status_change'::text, count(DISTINCT h.lead_id)::int
  ...
  WHERE h.changed_by IS NOT NULL
    ...
    AND h.new_status IN (SELECT st FROM pipeline)
  ...
  FROM public.showroom_visits v, bounds b
  WHERE v.salesperson_id IS NOT NULL
    AND v.created_at >= b.start_at AND v.created_at < b.end_at
