-- Detalle clicable de cada categoría del score diario.
...
BEGIN
  ...
  v_start := (p_fecha::timestamp AT TIME ZONE 'America/Guayaquil');
  ...
  IF p_categoria = 'lead_status_change' THEN
    ...
    FROM (
      SELECT
        ...
        ('Resumen · ' || coalesce(l.status::text, 'lead')) AS titulo,
        left(btrim(l.resume), 280) AS detalle,
        'lead'::text AS recurso
      FROM public.leads l
      WHERE l.assigned_to = p_vendedor_id
        AND l.resume_updated_at >= v_start AND l.resume_updated_at < v_end
...
REVOKE ALL ON FUNCTION public.get_sales_progress_events(date, uuid, text) FROM PUBLIC;
