-- La categoría lead_status_change pasa a ser leads contestados:
-- resumen ejecutivo guardado hoy (misma regla que Gestión de hoy en Leads).

UPDATE public.sales_progress_weights
SET label = 'Leads contestados (resumen)',
    daily_cap_points = 90
WHERE category = 'lead_status_change';

CREATE INDEX IF NOT EXISTS idx_leads_assigned_resume_updated
  ON public.leads (assigned_to, resume_updated_at DESC)
